...
const treinarPokemon = async (request, response) => {
  const authHeaderTreinar = request.get('authorization')
  let autenticado = false

  if (!authHeaderTreinar) {
    return response.status(401).send('Você precisa fazer login!')
  }

  const token = authHeaderTreinar.split(' ')[1]
...
const updatePokemon = async (request, response) => {
  const authHeaderUpdate = request.get('authorization')
  let autenticado = false
  ...
  const token = authHeaderUpdate.split(' ')[1]
...
const getPokemonById = async (request, response) => {
  const authHeaderPokById = request.get('authorization')
  let autenticado = false

  if (!authHeaderPokById) {
    return response.status(401).send('Você precisa fazer login!')
  }

  const token = authHeaderPokById.split(' ')[1]
